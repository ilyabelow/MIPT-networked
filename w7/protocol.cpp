...
{
  size_t size = ruler().start() << E_SERVER_TO_CLIENT_SET_CONTROLLED_ENTITY << PackUint{&eid};
  ENetPacket *packet = enet_packet_create(nullptr, size, ENET_PACKET_FLAG_RELIABLE);
  bitstream(packet->data).start() << E_SERVER_TO_CLIENT_SET_CONTROLLED_ENTITY << PackUint{&eid};
...
void send_entity_input(ENetPeer *peer, uint32_t eid, float thr, float ori)
{
  ...
  size_t size = ruler().start() << E_CLIENT_TO_SERVER_INPUT << PackUint{&eid} << controlsPacked;
  ENetPacket *packet = enet_packet_create(nullptr, size, ENET_PACKET_FLAG_UNSEQUENCED);
  bitstream(packet->data).start() << E_CLIENT_TO_SERVER_INPUT << PackUint{&eid} << controlsPacked;
...
void send_snapshot(ENetPeer *peer, uint32_t eid, float x, float y, float ori)
{
  ...
  size_t size = ruler().start() << E_SERVER_TO_CLIENT_SNAPSHOT << PackUint{&eid} << tmPacked;
  ENetPacket *packet = enet_packet_create(nullptr, size, ENET_PACKET_FLAG_UNSEQUENCED);
  bitstream(packet->data).start() << E_SERVER_TO_CLIENT_SNAPSHOT << PackUint{&eid} << tmPacked;
...
{
  bitstream(packet->data).start() >> Skip<MessageType>{} >> UnpackUint{&eid};
}

void deserialize_entity_input(ENetPacket *packet, uint32_t &eid, float &thr, float &steer)
{
  ControlsQuantized ctrlPacked(0);
  bitstream(packet->data).start() >> Skip<MessageType>{} >> UnpackUint{&eid} >> ctrlPacked;
...
void deserialize_snapshot(ENetPacket *packet, uint32_t &eid, float &x, float &y, float &ori)
{
  ...
  bitstream(packet->data).start() >> Skip<MessageType>{} >> UnpackUint{&eid} >> tmPacked;
